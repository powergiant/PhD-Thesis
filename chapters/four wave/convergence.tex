...
% In the last section of \ref{sec.uppcoef.fourwave}, we give an upper bounds for the coefficients of these Gaussian polynomials.
...
\begin{defn} \label{def.renorm.fourwave}
\begin{enumerate}
...
% \begin{equation}\label{eq.iotadef.fourwave}
...
The following table provides a dictionary between concepts in Definition \ref{def.tree.fourwave} in the previous section and expressions in tree terms $\mathcal{J}_{T,k}$ in Lemma \ref{lem.treeterms.fourwave}.
...
Lemma \ref{lem.treeterms.fourwave} suggests that the coefficients is small when $|\Omega_{\mathfrak{n}}|\gg \omega$, $\omega$ is supposed to be $\frac{1}{T_{\text{kin}}}$. Therefore, in order to bound $\mathcal{J}_{T,k}$, we should count the lattice points on $|\Omega_{\mathfrak{n}}|\lesssim \omega$
...
\begin{figure}[H]
 ...
 \label{fig.equations.fourwave}
 \end{figure}
 


 \subsection{Couples and the renormalized Wick theorem} \label{sec.coupwick.fourwave} In this section, we introduce the renormalized Wick theorem and calculate $\mathbb{E}|\mathcal{J}_{T,k}|^2$ using this theorem. We also introduce another type of diagram, the couple diagrams, to represent the result.
 ...
 %In this section, we derive the formula \eqref{eq.couples.fourwave} for calculating expectations $\mathbb{E}(\mathcal{J}_{T,k}\overline{\mathcal{J}_{T_{\text{in}},k'}})$ in terms of couples. 
 ...
 % \begin{equation}\label{eq.expectation''.fourwave}
 ...
 \begin{equation}\label{eq.expectationr.fourwave}
 ...
 \begin{defn}\label{def.pairing.fourwave}
 \begin{enumerate}
 ...
 \begin{thm}[Wick theorem]\label{th.wick.fourwave}
 ...
 \begin{thm}[Renormalized Wick theorem]\label{th.wickr.fourwave} In addition to assumptions in Theorem \ref{th.wick.fourwave}, define $\mathcal{P}_F=\mathcal{P}_F(A)$ (remember that $A=\{k_1,k_2,\cdots,k_{2m}\}$ and $\mathcal{P}_F(A)$ is defined in Definition \ref{def.pairing.fourwave} (5)), then 
 \begin{equation}
  \mathbb{E}([\eta^{\tau_1}_{k_1}\cdots \eta^{\tau_{2m}}_{k_{2m}}]_F)=\sum_{p\in \mathcal{P}_F} \delta_{p}(k_1,\cdots,k_{2m})
 \end{equation}
 \end{thm}
 % \begin{rem}\label{rem.renormwick.fourwave} The \textit{renormalized pairing set} $p\in \mathcal{P}_F$ is the set of pairings that do not pair all leaves of any subtree with leaves of the same subtree.
 % \end{rem}
 ...
 Applying Wick theorem to \eqref{eq.termexp1.fourwave}, we get
 ...
 The pairing $p$ in Wick theorem introduces new equations $k_{i}=k'_{j}$ (defined in \eqref{eq.deltapairing.fourwave}) and the coefficients $H^T_{k_1\cdots k_{2l(T)+1}} H^{T}_{k'_1\cdots k'_{2l(T)+1}} \delta_{p}$ concentrate near the subset 
 ...
 \begin{defn}[Construction of couples]\label{def.couple.fourwave}Given two trees $T$ and $T'$, we flip the orientation of all edges in $T'$ (as in the two left trees in Figure \ref{fig.treepairing.fourwave}). We also label their leaves by $1, 2, \cdots, 2l(T)+1$ and $1, 2, \cdots, 2l(T')+1$ so that the corresponding variables of these leaves are $k_1, k_2, \cdots, k_{2l(T)+1}$ and $k_1, k_2, \cdots, k_{2l(T')+1}$. Assume that we have a pairing $p$ of the set $\{k_1, k_2, \cdots, k_{2l(T)+1}, k_1, k_2, \cdots,$ $ k_{2l(T')+1}\}$, then this pairing induces a pairing between leaves (if $k_i\sim_p k_j$ then define $\textit{the $i$-th leaf}\sim_p \textit{the $j$-th leaf}$). Given this pairing of leaves, we define the following procedure which glues two trees $T$ and $T'$ into a couple $\mathcal{C}(T,T,p)$. Some examples of pairing can be found in Figure \ref{fig.treepairing.fourwave}. 
  ...
  \label{fig.treepairing.fourwave}
  \end{figure}
 
 \begin{enumerate}
  \item \textbf{Merging edges connected to leaves.} Given two edges with opposite orientations connected to two paired leaves, these two edges can be \underline{merged} into one edge as in Figure \ref{fig.pairingleaves.fourwave}. Since the pairing in this paper is a balanced pairing, only edges with opposite orientations can be paired.
  \begin{figure}[H]
  \centering
  \scalebox{0.5}{
  ...
  \label{fig.pairingleaves.fourwave}
  \end{figure}
  ...
  \item \textbf{Splitting of $\circ$ nodes: } Given a $\circ$ node $\mathfrak{n}$, we split it into two edges as in Figure \ref{fig.splitcirc.fourwave}.
  \begin{figure}[H]
  \centering
  \scalebox{0.3}{
  \begin{tikzpicture}[level distance=80pt, sibling distance=70pt]
  ...
  $\mathfrak{n}_1$ and $\mathfrak{n}_2$. Figure \ref{fig.splitcircexample.fourwave} is an example of splitting $\circ$ in a tree
  \begin{figure}[H]
  \centering
  \scalebox{0.3}{
  \begin{tikzpicture}[level distance=80pt, sibling distance=70pt]
  ...
  \label{fig.splitcircexample.fourwave}
  \end{figure}
  ...
  \item \textbf{Pairing of trees and couples.} Given a pairing $p$ of the set of leaves in $T$, $T'$ we merge all edges paired by $p$ as in Figure \ref{fig.couple.fourwave} and the resulting combinatorial structure is called a \underline{couple}. 
  \begin{figure}[H]
  \centering
  \scalebox{0.2}{
  \begin{tikzpicture}[level distance=80pt, sibling distance=70pt]
  ...
  \label{fig.couple.fourwave}
  \end{figure}
  
  We know that each edge connected to a leaf corresponds to a variable $k_i$. A pairing $p$ of $\{k_1,k_2,\cdots,k_{2m}\}$ in \eqref{eq.diophantineeqpaired.fourwave} induces a pairing of edges connected to leaves. Merging paired edges corresponds to $k_{i}=k'_{j}$ for all $k_{i}\sim_{p}k'_{j}$ in \eqref{eq.diophantineeqpaired.fourwave}. 
 \end{enumerate}
 \end{defn}
 ...
 \begin{prop}\label{prop.termcouple.fourwave} 
 ...
 % \begin{equation}\label{eq.couples.fourwave}
 % 1
 % \end{equation}
 % \end{lem}
 
 
\subsection{Counting lattice points}\label{sec.numbertheory.fourwave} In this section, we apply the connection between couple and concentration subset \eqref{eq.diophantineeqpaired.fourwave} to count the number of solutions of a generalized version of \eqref{eq.diophantineeqpaired.fourwave},
...
\begin{defn}\label{def.morecouple.fourwave}
\begin{enumerate}
...
\begin{figure}[H]
 ...
 \label{fig.exampleofcuttingidea.fourwave}
 \end{figure}

By \eqref{eq.diophantineeqpairedsigma'.fourwave}, we know that the couple $\mathcal{C}$ corresponds to the following equations.
\begin{equation}\label{eq.cuttingexmaple.fourwave}
 \begin{split}
...
We know that \eqref{eq.cuttingexmaple.fourwave} can be rewritten into the form $\bigcup_{a,e\in A} B_{a,e}$, where
...
With the above definition, it's easy to show that the couple $A$ and $B_{a,b,c}$ in Figure \ref{fig.exampleofcuttingidea.fourwave} correspond to the system of equations \eqref{eq.couplequationA.fourwave} and \eqref{eq.couplequationB.fourwave} respectively.
...
\begin{defn}
 \begin{enumerate}
 \item \textbf{Cutting an edge:} Given an edge $\mathfrak{e}$, we can cut it into two edges (a fixed and a free leg) as in Figure \ref{fig.cutedge.fourwave}.
...
\begin{lem}\label{lem.Eq(C)cutting.fourwave}
...
\begin{equation}\label{eq.Eq(C)cutting.fourwave}
...
\end{equation}
and
\begin{equation}\label{eq.Eq(C)cuttingcounting.fourwave}
...
\end{lem}
\begin{proof} By definition \eqref{eq.Eq(C,c).fourwave}, we have
\begin{equation}
\begin{split}
...
\begin{lem}\label{lem.freeleg.fourwave} %Free leg moving lemma
...
\begin{defn}
 ...
 \item \textbf{Good couples and bad couples.} A closed couple is a \underline{good couple} (resp. \underline{bad couple}) if it (resp. does not) contains at least two dotted edges (see Definition \ref{def.couple.fourwave} (2)). 
 
 Figure \ref{fig.goodbad.fourwave} provides some examples of couples that are good or bad.
 ...
 \label{fig.chainofbadnodes.fourwave}
 \end{figure}
 ...
 % Figure \ref{fig.reducibility.fourwave} provides some examples that do or do not satisfy this property.
 
 % \begin{figure}[H]
 % \centering
 % %\includegraphics[scale=0.25]{components_cutted.png}
 % \caption{Example of two edge reducible couples (left) and two edge reducible couples (right)}
 % \label{fig.reducibility.fourwave}
 % \end{figure}
 \end{enumerate}
\end{defn}
...
\begin{lem}\label{eq.ineqn_BCn_G.fourwave} Given two trees $T$, $T'$ and a pairing $p\in\mathcal{P}_F$ of leaves in the renormalized pairing set $\mathcal{P}_F$, let $F$ be the forest which is the union of two forests $R(T)$ and $R(T')$. Then the couple $\mathcal{C} = \mathcal{C}(T,T',p)$ constructed from $T$, $T'$, $p$ have the following properties.
...
% \begin{equation}\label{eq.n_B.fourwave}
% n_{B}(\mathcal{C}) = n_{B}(T)
% \end{equation}
% \begin{equation}\label{eq.n_G.fourwave}
...
% \end{lem}
% \begin{proof}
% We first prove \eqref{eq.n_B.fourwave}. From Definition \ref{def.couple.fourwave} (2), we know that one $\circ$ node corresponds to one dotted edge. We need the following claim to continue the proof.
...
% Given a bad node, since all leaves in $L(T_{\mathfrak{n}})\backslash\cup_{n'\in \text{Child}_{\mathfrak{n}}} L(T_{\mathfrak{n}'})$ are paired by $p$ with leaves in the same set, by the above claim, we know that the same is true for $L(T_{n_1})\cup L(T_{n_2})$. Since all leaves in $L(T_{n_1})\cup L(T_{n_2})$ are paired with each other, we get a closed component with only one dotted edge which is bad. All bad components arise in this way, this gives \eqref{eq.n_B.fourwave}.


% Then we prove \eqref{eq.n_G.fourwave}. Since each good components contain at least two dotted edges, and these two dotted edges correspond to two good nodes, so we know that $2n_{G}(\mathcal{C}) \le n_{G}(T)$. This gives \eqref{eq.n_G.fourwave}.

% Finally, we prove \eqref{eq.n_Bn_G.fourwave}.
...
% Apply the Lemma \ref{lem.momentumconservation.fourwave} to $\mathcal{C}_1$, then we know that $k_{\mathfrak{e}_1}=k_{\mathfrak{e}_2}$. By \ref{eq.momentumconservationunit.fourwave}, we know that $k_{\mathfrak{e}_1}\ne k_{\mathfrak{e}_2}\ne k_{\mathfrak{e}_3}$. Therefore, the set of solutions must be empty. 

% Now we prove (1). 

% After splitting all $\circ$ nodes as in Definition \ref{def.couple.fourwave} (3), $T$ is decomposed into $\{T_j\}$ and a tree $T_{\text{leg}}$, so that $T_{\text{leg}}$ contains the legs of $T$ and each $T_j$ contains exactly one dotted edge. It's easy to see that $\cup_{j} L(T_j)$ and $L(T_{\text{leg}})$ coincides with $L(F_{\mathfrak{n}})$ and $\{k_1,k_2,\cdots,k_{2m}\}\backslash L(F_{\mathfrak{n}})$ respectively. ($F=R(T)\cup R(T')$) Therefore, by Theorem \ref{th.wickr.fourwave} and Definition \ref{def.pairing.fourwave} (5), we know that there is no tree in $\{T_j\}$ and $T_{\text{leg}}$ whose leaves pair only with the leaves itself. Therefore, all trees in $\{T_j\}$ and $T_{\text{leg}}$ have at least one edge connected to another different tree. If a component does not have a leg, which means that $T_{\text{leg}}$ does not contain this component, then it contains at least two trees in $\{T_j\}$. This implies that this component has at least two dotted edges since each of these two $T_j$ trees contains a dotted edge.


% *****************************************************

% \textbf{Step 1.} In this step, we show that after splitting all $\circ$ nodes as in Definition \ref{def.couple.fourwave} (3), $T$ is decomposed into $\{T^{\mathfrak{n}}\}_{\mathfrak{n}\text{ is decorated by } \circ}$ and a tree $T_{\text{leg}}$, so that $T_{\text{leg}}$ contains the legs of $T$ and each $T^{\mathfrak{n}}$ contains exactly one dotted edge.

% Before doing any proof, we refine Definition \ref{def.couple.fourwave} (3) about splitting $\circ$ nodes when constructing couples. We eliminate its ambiguity by specifying the order of splitting $\circ$ nodes. 

% First split all $\circ$ nodes without $\circ$ descendants as in Definition \ref{def.couple.fourwave} (2). After splitting one such $\circ$ nodes $\mathfrak{n}_{\circ}$, we get one tree $T_{\text{leg}}^{1}$ containing the leg and another tree $T^{\mathfrak{n}_{\circ}}$ containing one dotted edge connecting two subtrees $T_{\mathfrak{n}_1}$, $T_{\mathfrak{n}_2}$. Notice that there are no $\circ$ nodes in $T^{\mathfrak{n}_{\circ}}$.
...
% By definition of $\mathcal{P}_{F}$ in Theorem \ref{th.wickr.fourwave}, 
...
\begin{prop}\label{prop.counting.fourwave}
...
% \begin{equation}\label{eq.dottededgeclosedcomponent.fourwave}
%     n_c\le \frac{1}{2}n_d
% \end{equation}
...
\end{prop}
\begin{proof} We first prove (2). The fact that $n$ equals the total number of $\bullet$ nodes in $T$ and $T'$ follows from the definition. The first inequality of \eqref{eq.ineqn_Gn_B.fourwave} follows from Lemma \ref{eq.ineqn_BCn_G.fourwave}. 
    ...
    As in Figure \ref{fig.3possibilities.fourwave}, there are three possibilities of $\mathcal{C}_{\mathfrak{n}}$. We label them by $\mathcal{C}_{I}$, $\mathcal{C}_{II}$, $\mathcal{C}_{III}$.
    ...
    \begin{figure}[H]
     ...
     \label{fig.3possibilities.fourwave}
    \end{figure}
    ...
    \begin{figure}[H]
     ...
     \label{fig.examplecase1.1.fourwave}
    \end{figure}
    ...
    \textbf{Case 3.1.} One of $\mathfrak{e}'$, $\mathfrak{e}''$, $\mathfrak{e}'''$ connects $\mathfrak{n}_*$ and $\mathcal{C}_1$ (w.l.o.g. assume that it's $\mathfrak{e}'$), another one connects $\mathfrak{n}_*$ and $\mathcal{C}_2$ (w.l.o.g. assume that it's $\mathfrak{e}''$) and the last one is a leg (w.l.o.g. assume that it's $\mathfrak{e}'''$). One example of this case is shown in Figure \ref{fig.examplecase3.1.fourwave}
    \begin{figure}[H]
     \centering
     \scalebox{0.4}{
     \begin{tikzpicture}[level distance=80pt, sibling distance=100pt]
     ...
     \label{fig.examplecase3.1.fourwave}
    \end{figure}
...
% \subsection{An upper bound of tree terms}\label{sec.treetermsupperbound.fourwave}
% In this section, we prove the following upper bound of the variance of $\mathcal{J}_{T,k}$.

% \begin{prop}\label{prop.treetermsvariance.fourwave}
...
% \end{prop}
% \begin{proof}
% By \eqref{eq.termTp.fourwave} and \eqref{eq.termexp.fourwave}, we know that $\mathcal{J}_{T,k}$ is a linear combination of $Term(T,p)$.
% \begin{equation}
% \begin{split}
%     \mathbb{E}|\mathcal{J}_{T,k}|^2=\left(\frac{\lambda^2}{L^{2d}}\right)^{2l(T)}
%     \sum_{p\in \mathcal{P}_F(\{k_1,\cdots, k_{2l(T)+1}, k'_1,\cdots, k'_{2l(T)+1}\})} Term(T, p).
...
% Since $\alpha=\frac{\lambda^2}{L^{d}}$, so $\frac{\lambda^2}{L^{2d}}=\alpha L^{-d}$. Since the number of elements in $\mathcal{P}_F$ can be bounded by a constant, by Lemma \ref{lem.Tpvariance.fourwave} proved below, we get
% \begin{equation}\label{eq.proptreetermsvariance1.fourwave}
% \begin{split}
...
% By definition, $n$ and $n_d$ are total number of $\bullet$ nodes and $\circ$ nodes respectively of the two paired trees. The two paired trees of the same type $T$ contains $2l(T)$ branching nodes in total (each one contains $l(T)$). We thus get $2l(T)=n+n_d$. Replacing $2l(T)$ by $n+n_d$ in \eqref{eq.proptreetermsvariance1.fourwave}, we get
% \begin{equation}
% \begin{split}
...
% \begin{lem}\label{lem.Tpvariance.fourwave} Assume that $\alpha\le L^{-2}$ and the initial data satisfies $\sqrt{n_{\textrm{in}}(k)}\lesssim \langle k\rangle^{-q}$. Let $\mathcal{C}$ be the couple constructed from tree $T$ and pairing $p$. Then for any $\theta>0$ and sufficiently large $q$ depending on $T$, we have
% \begin{equation}
%     \sup_k\langle k\rangle^q\, |(Term(T,p))_k|\le L^{O(\theta)} Q^{n} L^{\frac{1}{2} dn_d} t^{2l(T)}.
% \end{equation}
% Here $n$, $n_d$ are defined in Proposition \ref{prop.counting.fourwave}
% \end{lem}
% \begin{proof} By \eqref{eq.termTp.fourwave}, we get
...
% By \eqref{eq.boundcoef.fourwave}, we get
% \begin{equation}
...
% \end{equation}
% $\widetilde{Eq}(\mathcal{C}, \{\sigma_{\mathfrak{n}}\}_{\mathfrak{n}},k)$ is the same as $Eq(\mathcal{C})$ defined in \eqref{eq.diophantineeqpairedsigma'.fourwave}, except that $k_{\mathfrak{e}}$ may not satisfy $|k_{\mathfrak{e}}| \lesssim L^+$ . With this definition, we know that 
% \begin{equation}
...
% \begin{equation}\label{eq.lemboundtermTp.fourwave}
% \begin{split}
%     &Term(T, p)
%     \\
...
% \begin{equation}\label{eq.lemboundtermTpsum.fourwave}
% \begin{split}
...
% \end{split}
% \end{equation}
% Here in the last inequality we applied \eqref{eq.countingbd1.fourwave} and $n$, $n_d$ were defined in Proposition \ref{prop.counting.fourwave}.

% Combining \eqref{eq.lemboundtermTp.fourwave} and \eqref{eq.lemboundtermTpsum.fourwave}, we get
% \begin{equation}
% \begin{split}
%     &Term(T, p)
%     \\
...
% We claim that 
% \begin{equation}\label{eq.lemTpvarianceclaim.fourwave}
...
% Before proving \eqref{eq.lemTpvarianceclaim1.fourwave}, let's first look at one of its special case. If $c,c'=Id$, then $c(\{m_{h}\})_{h}=c'(\{m_{h}\})_{h}=m_h$. The right hand side of \eqref{eq.lemTpvarianceclaim1.fourwave} becomes
% \begin{equation}
% \begin{split}
...
% Now we prove \eqref{eq.lemTpvarianceclaim1.fourwave}. Use Cauchy inequality we get
% \begin{equation}
% \begin{split}
...
\subsubsection{Estimates of tree terms} By \eqref{eq.approxerror.fourwave}, in order to control the approximation error $Err$, it suffices to get upper bounds of tree terms $\mathcal{J}_T$.
...
Therefore, we just need to consider the order of magnitude of $Term(T,p)$.
...
But remember that Proposition \ref{prop.counting.fourwave} (3) made the assumption that there is no long chain of bad circle nodes. This is the second essential gap.
...
\begin{figure}[H]
 ...
 \node[draw, circle, minimum size=1cm, scale=2] at (0,0) (1) {$T_{0}$} 
 child {node[circ] (11) {} 
 child {node[draw, circle, minimum size=1cm, scale=2] (111) {$T_{1}$}
 child {node[circ, xshift = -3cm] (1111) {} 
 child {node[draw, circle, minimum size=1cm, scale=2] (11111) {$T_{2}$}
 }
 child {node[draw, circle, minimum size=1cm, scale=2] (11112) {$T_{2}'$}
 }
 child {node[draw, circle, minimum size=1cm, scale=2] (11113) {$T_{2}^e$}
 }
 }
 }
 child {node[draw, circle, minimum size=1cm, scale=2] (112) {$T_{1}'$}
 }
 child {node[draw, circle, minimum size=1cm, scale=2] (113) {$T_{1}^e$}
 }
 };
 
 \node[scale =4] at ($(11112)+(0,-3)$) {$\cdots\cdots$};
 \draw[-{Stealth[length=5mm, width=3mm]}] (1) -- (11);
 \draw[-{Stealth[length=5mm, width=3mm]}] (11) -- (111);
 \draw[{Stealth[length=5mm, width=3mm]}-] (11) -- (112);
 \draw[-{Stealth[length=5mm, width=3mm]}] (11) -- (113);
 \draw[-{Stealth[length=5mm, width=3mm]}] (111) -- (1111);
 \draw[-{Stealth[length=5mm, width=3mm]}] (1111) -- (11111);
 \draw[{Stealth[length=5mm, width=3mm]}-] (1111) -- (11112);
 \draw[-{Stealth[length=5mm, width=3mm]}] (1111) -- (11113);

 \node[] at ($(11111)+(-3,-6)$) (2) {} 
 child {node[circ, xshift = -3cm] (21) {} 
 child {node[draw, circle, minimum size=1cm, scale=2] (211) {$T_{k}$}
 }
 child {node[draw, circle, minimum size=1cm, scale=2] (212) {$T_{k}'$}
 }
 child {node[draw, circle, minimum size=1cm, scale=2] (213) {$T_{k}^e$}
 }
 };
 ...
 \node[draw, circle, minimum size=1cm, scale=2] at ($(1)+ (10, 0)$) (3) {$T_{0}'$};
 \end{tikzpicture}
 }
 \caption{Long chain of bad circle nodes}
 \label{fig.chainofbad.fourwave}
\end{figure}

In Proposition \ref{prop.counting.fourwave}, we have the desired counting result if there is no long chain of bad nodes. For each long chain of bad nodes, we lose a factor of $\alpha^{-1}$ for each group $\{T_i, T_i', T_i^e\}$ ($1\le i\le k-1$). However, if the pairing within the group $\{T_i, T_i', T_i^e\}$ is not regular (after splitting the circle node), then by the argument of Deng and Hani \cite{deng2021full}, \cite{deng2023derivation}, we gain a factor which can be used to compensate the loss. If the pairing is regular, we have the following cancellation identity.

After splitting, a circle node gives a dotted edge. In a regular pairing, there are only three possibilities for the configuration of this dotted edge. As shown in the following three pictures.
...
\begin{figure}[H]
 ...
 child{
 node[fillcirc] (11) {}
 child{node[fillstar] (111) {} edge from parent[draw=none]}
 child{node[fillstar] (112) {}}
 child{node[fillstar] (113) {}}
 };
 \draw[-{Stealth[length=5mm, width=3mm]}] (1) -- (11);
 \draw[-{Stealth[length=5mm, width=3mm]}, dashed] (11) -- (111);
 \draw[{Stealth[length=5mm, width=3mm]}-] (11) -- (112);
 \draw[-{Stealth[length=5mm, width=3mm]}] (11) -- (113);
 \node[scale = 2.0] at ($(1) + (0.5, -0.2)$) {$k$};
 \node[scale = 2.0] at ($(111) + (0.7, 0)$) {$k_1$};
 \node[scale = 2.0] at ($(112) + (0.7, 0)$) {$k_2$};
 \node[scale = 2.0] at ($(113) + (0.7, 0)$) {$k_3$};
 \node[scale = 2.0] at ($(11) + (0.8, 0)$) {$t_1$};
 \node[scale = 2.0] at ($(11) + (-1.0, -1.8)$) {$t_3$};

 \node[] at (10,0) (2) {}
 child{
 node[fillcirc] (21) {}
 child{node[fillstar] (211) {} edge from parent[draw=none]}
 child{node[fillstar] (212) {}}
 child{node[fillstar] (213) {}}
 };
 \draw[{Stealth[length=5mm, width=3mm]}-] (2) -- (21);
 ...
 \draw[-{Stealth[length=5mm, width=3mm]}] (21) -- (212);
 \draw[{Stealth[length=5mm, width=3mm]}-] (21) -- (213);
 \node[scale = 2.0] at ($(2) + (0.5, -0.2)$) {$k$};
 \node[scale = 2.0] at ($(211) + (0.7, 0)$) {$k_1$};
 \node[scale = 2.0] at ($(212) + (0.7, 0)$) {$k_2$};
 \node[scale = 2.0] at ($(213) + (0.7, 0)$) {$k_3$};
 \node[scale = 2.0] at ($(21) + (0.8, 0)$) {$t_2$};

 \draw[bend right =30, dashed] (111) edge (211);
 \draw[bend right =30, dashed] (112) edge (212);
 \draw[bend right =30, dashed] (113) edge (213);

 \node[rectangle, draw, minimum width = 20cm, minimum height = 8cm] at (5,-3.7) {}; 

 \node[] at (25,0) (3) {}
 child{
 node[fillcirc] (31) {}
 child{node[fillstar] (311) {} edge from parent[draw=none]}
 child{node[fillstar] (312) {}}
 child{node[fillstar] (313) {}}
 };
 \draw[-{Stealth[length=5mm, width=3mm]}] (3) -- (31);
 ...
 \draw[{Stealth[length=5mm, width=3mm]}-] (31) -- (312);
 \draw[-{Stealth[length=5mm, width=3mm]}] (31) -- (313);
 \node[scale = 2.0] at ($(3) + (0.5, -0.2)$) {$k$};
 \node[scale = 2.0] at ($(311) + (0.7, 0)$) {$k_1$};
 \node[scale = 2.0] at ($(312) + (0.7, 0)$) {$k_2$};
 \node[scale = 2.0] at ($(313) + (0.7, 0)$) {$k_3$};
 \node[scale = 2.0] at ($(31) + (0.8, 0)$) {$t_2$};

 \node[] at (35,0) (4) {}
 child{
 node[fillcirc] (41) {}
 child{node[fillstar] (411) {} edge from parent[draw=none]}
 child{node[fillstar] (412) {}}
 child{node[fillstar] (413) {}}
 };
 \draw[{Stealth[length=5mm, width=3mm]}-] (4) -- (41);
 \draw[{Stealth[length=5mm, width=3mm]}-, dashed] (41) -- (411);
 \draw[-{Stealth[length=5mm, width=3mm]}] (41) -- (412);
 \draw[{Stealth[length=5mm, width=3mm]}-] (41) -- (413);
 \node[scale = 2.0] at ($(4) + (0.5, -0.2)$) {$k$};
 \node[scale = 2.0] at ($(411) + (0.7, 0)$) {$k_1$};
 \node[scale = 2.0] at ($(412) + (0.7, 0)$) {$k_2$};
 \node[scale = 2.0] at ($(413) + (0.7, 0)$) {$k_3$};
 \node[scale = 2.0] at ($(41) + (0.8, 0)$) {$t_1$};
 \node[scale = 2.0] at ($(41) + (-1.0, -1.8)$) {$t_3$};

 \draw[bend right =30, dashed] (311) edge (411);
 \draw[bend right =30, dashed] (312) edge (412);
 \draw[bend right =30, dashed] (313) edge (413);

 \node[rectangle, draw, minimum width = 20cm, minimum height = 8cm] at (30,-3.7) {}; 
 \end{tikzpicture}
 
 }
 \caption{The first configuration of the dotted edge and its conjugation}
 \label{fig.config1.fourwave}
\end{figure}
...
\begin{figure}[H]
 ...
 child{
 node[fillcirc] (11) {}
 child{node[fillcirc] (111) {} edge from parent[draw=none]
 child{node[fillstar] (1111) {}}
 child{node[fillstar] (1112) {}}
 child{node[fillstar] (1113) {}}
 }
 ...
 \draw[bend right =50, dashed] (1111) edge (21);
 \draw[bend right =30, dashed] (1112) edge (112);
 \draw[bend right =30, dashed] (1113) edge (113);

 \node[rectangle, draw, minimum width = 20cm, minimum height = 11cm] at (3,-4.9) {}; 

 \node[] at (35,0) (3) {}
 child{
 node[fillcirc] (31) {}
 child{node[fillcirc] (311) {} edge from parent[draw=none]
 child{node[fillstar] (3111) {}}
 child{node[fillstar] (3112) {}}
 child{node[fillstar] (3113) {}}
 }
 ...
 \draw[-{Stealth[length=5mm, width=3mm]}] (3) -- (31);
 \draw[-{Stealth[length=5mm, width=3mm]}, dashed] (31) -- (311);
 \draw[{Stealth[length=5mm, width=3mm]}-] (31) -- (312);
 \draw[-{Stealth[length=5mm, width=3mm]}] (31) -- (313);
 \draw[-{Stealth[length=5mm, width=3mm]}, dashed] (311) -- (3111);
 \draw[{Stealth[length=5mm, width=3mm]}-] (311) -- (3112);
 \draw[-{Stealth[length=5mm, width=3mm]}] (311) -- (3113);
 ...
 \draw[{Stealth[length=5mm, width=3mm]}-] (4) -- (41);
 \draw[bend left =50, dashed] (3111) edge (41);
 \draw[bend right =30, dashed] (3112) edge (312);
 \draw[bend right =30, dashed] (3113) edge (313);

 \node[rectangle, draw, minimum width = 20cm, minimum height = 11cm] at (31,-4.9) {}; 
 \end{tikzpicture}
 }
 \caption{The second configuration of the dotted edge and its conjugation}
 \label{fig.config2.fourwave}
\end{figure}
...
\begin{figure}[H]
 ...
 node[fillcirc] (11) {}
 child{node[fillcirc] (111) {}
 child{node[fillstar] (1111) {} edge from parent[draw=none]}
 child{node[fillstar] (1112) {}}
 child{node[fillstar] (1113) {}}
 }
 child{node[fillstar] (112) {}}
 ...
 \draw[{Stealth[length=5mm, width=3mm]}-] (11) -- (112);
 \draw[-{Stealth[length=5mm, width=3mm]}] (11) -- (113);
 \draw[-{Stealth[length=5mm, width=3mm]}, dashed] (111) -- (1111);
 \draw[{Stealth[length=5mm, width=3mm]}-] (111) -- (1112);
 \draw[-{Stealth[length=5mm, width=3mm]}] (111) -- (1113);
 ...
 node[fillcirc] (31) {}
 child{node[fillcirc] (311) {}
 child{node[fillstar] (3111) {} edge from parent[draw=none]}
 child{node[fillstar] (3112) {}}
 child{node[fillstar] (3113) {}}
 }
 child{node[fillstar] (312) {}}
 ...
 \caption{The third configuration of the dotted edge and its conjugation}
 \label{fig.config3.fourwave}
\end{figure}

We show that terms arising from conjugated configuration cancel with each other. We only consider the first configuration, because the calculation of other configurations is the same.
...
When $t_3, t\rightarrow \infty$ and sum over $k_1$, $k_2$, $k_3$, the two sums have the same asymptotic and thus cancel with each other.

% \subsubsection{Difficulties and } coefficients


% \subsection{An upper bound of coefficients in expansion series}\label{sec.uppcoef.fourwave} In this section, we derive an upper bound for coefficients $H^T_{k_1\cdots k_{2l+1}}$.

% Notice that in \eqref{eq.coefterm.fourwave}, $H^T_{k_1\cdots k_{l+1}}$ are integral of some oscillatory functions. An upper bound can be derived by the standard integration by parts arguments.

% Associate each $\mathfrak{n}\in T_{\text{in}}$ with two variables $a_{\mathfrak{n}}$, $b_{\mathfrak{n}}$. Then we define
% \begin{equation}\label{eq.defF_T.fourwave}
...
% \begin{lem}\label{lem.boundcoef'.fourwave}
...
% \begin{equation}\label{eq.boundcoef'.fourwave}
...
% \begin{equation}\label{eq.q_n'.fourwave}
%     q_{\mathfrak{n}}=
%     \begin{cases}
...
% For a tree $T$ contains only one node $\mathfrak{r}$, $F_{T}=1$ and \eqref{eq.boundcoef'.fourwave} is obviously true.

% Assume that \eqref{eq.boundcoef'.fourwave} is true for trees with $\le n-1$ nodes. We prove the $n$ nodes case. 

% For general $T$, let $T_1$, $T_2$, $T_3$ be the three subtrees and $\mathfrak{n}_1$, $\mathfrak{n}_2$, $\mathfrak{n}_3$ be the three children of the root $\mathfrak{r}$, then by the definition of $F_T$\eqref{eq.defF_T.fourwave}, we get
% \begin{equation}
% \begin{split}
...
% % \begin{equation}\label{eq.lemboundcoefexpand.fourwave}
% % \begin{split}
...
% \begingroup
% \allowdisplaybreaks
% \begin{align}\label{eq.lemboundcoefexpand.fourwave}
%     &F_{T}(t)=\frac{i}{a_{\mathfrak{r}}+\alpha \text{sgn}(a_{\mathfrak{r}}) }\int_{\cup_{\mathfrak{n}\in T_{\text{in}}} A_{\mathfrak{n}}} \frac{d}{dt_{\mathfrak{r}}}e^{-i(t_{\mathfrak{r}}a_{\mathfrak{r}}+\alpha\, \text{sgn}(a_{\mathfrak{r}}))} \notag
%     \\
...
% Here $T^{(j)}$, $j=1,2,3$ are trees that is obtained by deleting the root $\mathfrak{r}$, adding edges that connecting $\mathfrak{n}_j$ with other two nodes and defining $\mathfrak{n}_j$ to be the new root. For $T^{(j)}$, we can define the term $F_{T^{(j)}}$ by \eqref{eq.defF_T.fourwave}. It can be shown that $F_{T^{(j)}}$ defined in this way is the same as the $\int_{\cup_{\mathfrak{n}\in T_{\text{in}}} A_{\mathfrak{n}},\ t_{\mathfrak{r}}=t_{\mathfrak{n}_j}}$ term in the second equality of \eqref{eq.lemboundcoefexpand.fourwave}, so the last equality of \eqref{eq.lemboundcoefexpand.fourwave} is true. $F_{I}$ is the $\int_{\cup_{\mathfrak{n}\in T_{\text{in}}} A_{\mathfrak{n}},\ t_{\mathfrak{r}}=t}$ term and $F_{II}$ is the last term containing $\frac{d}{dt_{\mathfrak{r}}}$.

% We can apply the induction assumption to $F_{T^{(j)}}$ and show that $\frac{i}{a_{\mathfrak{r}}+\alpha \text{sgn}(a_{\mathfrak{r}})} F_{T^{(j)}}$ can be bounded by the right hand side of \eqref{eq.boundcoef'.fourwave}.

% A direct calculation gives that 
% \begin{equation}
%     F_{I}(t)=e^{-i(ta_{\mathfrak{r}}+\alpha c(t)b_{\mathfrak{r}})} F_{T_1}(t)F_{T_2}(t)F_{T_3}(t).
% \end{equation}
% Then the induction assumption implies that $\frac{i}{a_{\mathfrak{r}}+\alpha \text{sgn}(a_{\mathfrak{r}})} F_{I}$ can be bounded by the right hand side of \eqref{eq.boundcoef'.fourwave}.
...
% Combining the bounds of $F_{I}$, $F_{T^{(1)}}$, $F_{T^{(2)}}$, $F_{T^{(3)}}$, $F_{II}$, we conclude that $F_T$ can be bounded by the right hand side of \eqref{eq.boundcoef'.fourwave} and thus complete the proof of Lemma \ref{lem.boundcoef'.fourwave}.
% \end{proof}
 
% % For general $T$, let $T_1$, $T_2$, $T_3$ be the three subtrees of the root, then we have the following recursive formula for $F_{T}$
% % \begin{equation}\label{eq.lemboundcoefrecur.fourwave}
...
% \end{lem}
% \begin{proof}
% This is a direct corollary of Lemma \ref{eq.boundcoef'.fourwave} if we take $a_{\mathfrak{n}}=\Omega_{\mathfrak{n}}$, $b_{\mathfrak{n}}=\widetilde{\Omega}_{\mathfrak{n}}/(2\alpha \int^t_{0}m(s) ds)$ and $c(t_{\mathfrak{n}})=2\int^t_{0}m(s) ds$. 
% \end{proof}
