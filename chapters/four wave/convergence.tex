...
\begin{defn}
    \begin{enumerate}
        ...
        \end{figure}
        %Let $n_{G}(\mathcal{C})$ and $n_{B}(\mathcal{C})$ be the number of good and bad component of $\mathcal{C}$ respectively. Let $n_{G}(F)$ and $n_{B}(F)$ be the of good and bad nodes of $F$ respectively. Let $n_{BC}(\mathcal{C})$, $n_{BC}(\mathcal{F})$ chain of bad components and chain of bad nodes.
...
% For different chains of bad circle, the good children is different, and then we can define a bijection map from the set of chains of bad nodes to good nodes.
...
\end{prop}
\begin{proof} We first prove (2). The fact that $n$ equals to the total number of $\bullet$ nodes in $T$ and $T'$ follows from the definition. The first inequality of \eqref{eq.ineqn_Gn_B} follows from Lemma \ref{eq.ineqn_BCn_G}. 
...
\begin{proof}
If there are only one node in $\mathfrak{n}\in \mathcal{N}_{leg}$, then all fixed legs are connect to this node. By this, we know that not all free legs are connected to this node, because by assumption of case 1.1, not all legs are connected to one node. We choose $\mathfrak{n}_*$ to be this node. 
