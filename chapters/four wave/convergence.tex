...
The following table provide a dictionary between concepts in Definition \ref{def.tree} in previous section and expressions in tree terms $\mathcal{J}_{T,k}$ in Lemma \ref{lem.treeterms}.
...
Lemma \ref{lem.boundcoef} suggests that the coefficients is small when $|q_{\mathfrak{n}}|\gg \alpha$. Therefore, in order to bound $\mathcal{J}_{T,k}$, we should count the lattice points on $|q_{\mathfrak{n}}|\lesssim \alpha$
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[level distance=80pt, sibling distance=80pt]
        ...
        \node[scale=2.0] at (0,-7.5) {$\Lambda_{k_{\mathfrak{e}_1}}-\Lambda_{k_{\mathfrak{e}_2}}+\Lambda_{k_{\mathfrak{e}_3}}-\Lambda_{k_{\mathfrak{e}}}=O(\alpha^{-1})$};
...
The goal of the next two sections is to count the number of solutions of a modified version of above equation \eqref{eq.diophantineeq'}. 
...
\begin{enumerate}
    ...
    %In what follows, we put a bar $\bar{a}$ on an element $a\in A$ to indicate that it has negative sign and elements without bar have positive sign. Using this notation, $A$ can be written as $A=\{a_1,\cdots,a_{m}, \bar{b}_1, \cdots,\bar{b}_n\}$.
...
\end{equation}
\end{thm}
% \begin{rem}\label{rem.renormwick} The \textit{renormalized pairing set} $p\in \mathcal{P}_F$ is the set of pairings that do not pairs all leaves of any subtree with leaves of the same subtree.
% \end{rem}

\begin{proof} Before starting the formal proof, we give a proof for an example to demonstrate the idea.
...
\begin{enumerate}
    \item \textbf{Merging edges connected to leaves.} Given two edges with opposite orientation connected to two paired leaves, these two edges can be \underline{merged} into one edge as in Figure \ref{fig.pairingleaves}. Since the pairing in this paper is a balanced pairing, only edges with opposite orientation can be paired.
    \begin{figure}[H]
        \centering
        \scalebox{0.5}{
...
\begin{equation}\label{eq.cuttingexmaple}
    \begin{split}
        ...
        \\
        &a - b + c = k,\ \Lambda(a) - \Lambda(b) + \Lambda(c) - \Lambda(k) =\sigma_{1} + O(\alpha)
        \\
...
\begin{defn}
    \begin{enumerate}
        \item \textbf{Open and closed couples.} A connected couple is said to be \underline{open} (resp. \underline{closed}) if it (resp. does not) contains legs. A connected components of a couple is a \underline{open component} (resp. \underline{closed component}) if it is open (resp. closed).
...
We have the following lemma that gives equalities and inequalities between the number of good and bad components and nodes. 
...
We have the following lemma that gives an uppper bound on the number of components in terms of number of dotted edges in a couple and exclude the presence of two edge reducible components.
...
\end{equation}
\end{prop}
\begin{proof} Because $\mathcal{C}$ is admissible, by Proposition \ref{prop.termcouple} (1) or Definition \ref{def.morecouple} (1), each connected components of $\mathcal{C}$ contains at least two dotted edges. Therefore, we have $2n_{c}\le n_d$, which gives \eqref{eq.dottededgeclosedcomponent}.
...
% The couple $C$ may not be connected. May reduce to connected case. Need to show that

% (1) The only disconnectness comes from $\circ$ nodes (all connected components that does not contain the legs must contain dotted edge from a $\circ$ nodes) (assign dotted edges to closed connected components)

% (2) The connected component containing legs is open, others are closed. 

% (3) Removing a dotted edge gives a factor of $Q$ (in the equation the weights is $t/L^d$). 

% (4) By proposition below, the open connected component does not lose factor. The others lose a factor of $L^d$ and gain a factor of $Q^{\#\{\textit{dotted edges}\}}$. (1) implies that $\#\{\textit{dotted edges}\}\ge 1$, renormalized implies that $\#\{\textit{dotted edges}\}\ge 2$. Therefore in all other connected components we lose a factor $L^d Q^2\le 1$, so we do not lose factor in any components. 

% (5) Combine the results in each components we prove the proposition.
...
\textbf{Case 3.2.2.} Assume that $\mathcal{C}_1$ contains no legs or leaf edges. Then $\mathcal{C}_2$ must contain leaf edges, since $\mathcal{C}$ contains leaf edges. 

We apply the same argument as case 2 and case 3.1. Cut $\mathfrak{e}'''$ into $\mathfrak{e}'''^{(1)}\in  \mathcal{C}_{\mathfrak{n}_*}\cup \mathcal{C}_1$ and $\mathfrak{e}'''^{(2)}\in  \mathcal{C}_2$. Let $\mathfrak{e}'''^{(1)}$ be a leaf edge and $\mathfrak{e}'''^{(2)}$ be a leg. Cutting in this way, all couples contain both leaf edges and legs
As in case 3.1, we have
...
$\mathcal{C}_{\mathfrak{n}_*}\cup \mathcal{C}_1$ has only one leaf edges $\mathfrak{e}'''^{(1)}$ and a leg $\mathfrak{l}$ and both of these two edges are connected to $\mathfrak{n}_*$. Therefore, $\mathcal{C}_{\mathfrak{n}_*}\cup \mathcal{C}_1$ satisfies exactly the condition of case 1.2. Applying the conclusion there gives
\begin{equation}\label{eq.case3.2.2expand'}
